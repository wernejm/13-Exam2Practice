...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        if len(contents) <= volume:
            ...
            self.contentscopy = contents
            self.volume = volume

            self.ogcontents = contents
            self.ogvolume = volume
    ...
    def append_string(self, additional_contents):
        ...
        if (len(self.contents) + len(additional_contents)) > self.volume:
            for k in range(self.volume - len(self.contents), len(additional_contents), 1):
                self.extra = self.extra + additional_contents[k]
        ...
        self.contentscopy = self.contents
        return self.extra
    ...
    def shrink(self, new_volume):
        ...
        if new_volume >= len(self.contents):
            ...
            self.contents = ''
            self.volume = new_volume
            self.append_string(self.contentscopy)
            self.contentscopy = self.contents
            return self.extra
    ...
    def double_then_shrink(self, new_volume):
        ...
        self.discarded = self.discarded + len(self.extra)
    ...
    def reset(self):
        ...
        self.historycontainer = self.historycontainer + [self.contents]
        self.contents = self.ogcontents
        self.volume = self.ogvolume
    ...
    def combined_box(self, other_box):
        ...
        new_box = Box(self.contents + other_box.contents, len(self.contents) + len(other_box.contents))
        return new_box
